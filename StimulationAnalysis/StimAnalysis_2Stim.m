...
function sessiondata = StimAnalysis_2Stim(baseInfo,StimInfo,basePath,StimPath)
binAll = 1000; %[in sec]
binSmall = 8;%[in sec]
...
for stim = 1:nbstim
    ...
    [nameStimEl, ~] = generateElectrodeName(StimData);
...
Stimdiff = cell(currNbWell, 1); %Stimdiff = cell(currNbWell, 5);

for well = 1:currNbWell
    data = dataStim(well,:);
    %Stimdiff{well,1} = abs(data{1,2}./data{1,1}-1);  (needed fpr PI; just valide for 3 stimulations)   
    %Stimdiff{well,2} = abs(data{1,3}./data{1,1}-1);
    %Stimdiff{well,3} = abs(data{1,3}./data{1,2}-1); 
    %Stimdiff{well,4} = data{1,3}./data{1,1}-1; 
    Stimdiff{well,1} = data{1,2}./data{1,1}-1;
end
...
PSTHchange = cell(nbMaxEl, currNbWell); %PSTH area difference Stim2/Stim1
...
for well = 1:currNbWell
    ...
    %% calculating PI: just valide with 3 stimulations
    %Stim3/Stim1 absolut value
%     data1 = Stimdiff{well,1};
...
%     PI{well,2} = PI_;
    ...
    PSTHchange(1:changing,well) = num2cell(data3); %fold increase/decrease per active electrode  
...
sessiondata.PSTHareaDiff_Stim2To1=PSTHchange;
