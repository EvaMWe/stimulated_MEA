...
PSTHdiffGr = cell(maxNbEl,nbG,nbSessions);
...
%     PI2 = result.potentiationIndex(:,2);
      PSTHdiff = result.PSTHareaDiff_Stim2To1;
...
%         PI2_result(s+2,idxNames(gr):idxNamesEnd(gr)) = num2cell(temp);
        ...
        temp = PSTHdiff(:,numberedCl);
        temp = num2cell(conversion(temp,2));
        PSTHdiffGr(1:length(temp),gr,s) = temp;
...
m = size(PSTHdiffGr,1)*size(PSTHdiffGr,3);
%n = size(PSTHdiffGr2,1)*size(PSTHdiffGr2,3);
PSTHdiffRes = cell(m,nbG);
%PSTHdiffRes2 = cell(n,nbG);
for gr = 1:nbG
    temp = (conversion(PSTHdiffGr(:,gr,:),2))';
    temp_ = permute(temp, [1 3 2]);
    PSTHdiffRes(1:length(temp_),gr) = num2cell(temp_); 
...
logI = sum(conversion(PSTHdiffRes,1),2);
logI(logI ~= 0) = 1;
PSTHdiffRes=PSTHdiffRes(logical(logI),:);
...
stimResult(1).PSTHdiffRes2 = PSTHdiffRes;
